	...
	else
	{
		...
		if($statement->rowCount() > 0)
		{
			...
			foreach($result as $row)
			{
				...
				{
					$message = '<div class="alert alert-info">Your Email Address is not verify, so first verify your email address by click on this <a href="resend_email_otp.php"><b>Recieve OTP</b></a></div>';
				}
				else
				{
					...
					$statements = $connect->prepare($sub_query);
					...
					if($mail->Send())
					{
						echo '<script type="text/javascript">alert("Please Check Your Email for password reset code \n Note: Using server mail to send all emails to your inbox sometime take time but you will receive shortly, the register OTP on ur mail will be: '.$user_otp.'")</script>';

						echo '<script type="text/javascript">window.location="forget_password.php?step2=1&code=' . $row["user_activation_code"] . '"</script>';
					}
								else
			{
		...
		<div class="container">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<form method="POST">
						...
						<div class="form-group" align="right">
							...
							<input type="submit" name="check_otp" class="btn btn-success" value="Next" />
						</div>
					</form>
					<?php
					}
				...
				<div style="display: flex;width: 100%;position: relative;">
					...
					<div style="padding-right: 15px;"><b><a href="index.php">Register</a></b></div>
					</div>
			</div>
		</div>
		<br />
